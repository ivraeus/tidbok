...
class Selector {
  constructor(elm, opt = {}) {
    ...
    this.height = elm.offsetHeight * this.ratio;
    this.width  = elm.offsetWidth * this.ratio;
    // Defaults
    this.pos  = 0;                            // current position
    this.val  = 0;                            // Start value
    this.min  = 1/-0;                         // Minimum value
    this.max  = 1/0;                          // Maximum value
    this.tag  = v => {return v};              // Tag value callback
    this.step = 1;                            // Step value size
    this.unit = 100;                          // Step width
    // this.span = {min: 1/-0, max: 1/0};     // Enabled span
    this.loop = false;                        // Selector loop?
    this.call = false                         // Input callback
    // Set options
    ...
    this.unit *= this.ratio;
    this.units = Math.ceil(this.width / this.unit);

    this.elm.width = this.width;
    this.elm.height = this.height;
    this.ctx.translate(Math.round(this.width / 2), 0);
    this.ctx.font = "40px Arial";

    this.paint();
    elm.addEventListener('touchstart', e => this.onTouchStart(e));
  }
  ...
  pullLoop() {
    if (this.pulling) {
      this.paint();
      requestAnimationFrame(() => this.pullLoop());
    }
  };
  ...
  paint() {
    let pos = (this.pos % this.unit) - 2;

    this.ctx.clearRect(-this.width / 2, 0, this.width, this.height);


    for (var i = 0; i < this.units / 2 + 1; i++) {

      let index = Math[this.pos < 0 ? 'floor' : 'ceil'](
        (-this.pos / this.unit) % this.unit);

      this.ctx.fillStyle = '#000';

      if (this.step * (index - i) % 1 === 0) {
        this.ctx.fillText(this.step * (index - i), i * -this.unit + pos, 40);
      }
      ...
      this.ctx.fillRect(i * -this.unit + pos, 60, 4, this.height);
      this.ctx.fillRect(i * +this.unit + pos, 60, 4, this.height);
  ...
  onTouchStart(event) {
    ...
    let lastPos = event.touches[0].pageX * this.ratio;

    this.pullLoop();

    let onTouchMove = event => {
      let newPos = event.touches[0].pageX * this.ratio;
      this.pos += newPos - lastPos
      lastPos = newPos;
    }
...
let monthSelector = new Selector(
  ...
  {
    val:  8,          // Start value
    min:  0.25,       // Minimum value
    max:  13,         // Maximum value
    step: 0.25,       // Step value size
    unit: 20,         // Step width
    loop: 1,          // should selector loop?
    call: v => {      // Input callback
